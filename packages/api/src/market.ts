...
export function mountMarket(app: Hono, db: Db, network: string, log: Logger, opts: { circle?: CircleCatalogue | null } = {}): { circle: CircleCatalogue | null; catalogue: (origin: string) => Promise<Catalogue> } {
  ...
  app.get("/v1/market/search", async (c) => {
    ...
    return c.json({
      ...
      searched: { ownRoutes: own.length, marketListings: market.length, circleCatalogue: listedByCircle.length },
      circleReadAt: circleRead ? Math.floor(circleRead / 1000) : null,
      sources: {
        "cra-agent": "CRA AGENT's own routes.",
        market: "Listings on cra-agent.tech/market. We call each one every hour to check it answers 402 on Arc.",
        circle: `Circle's x402 catalogue, the list behind its Agent Marketplace (${CIRCLE_DISCOVERY}), entries payable on Arc${circleRead ? `, read ${new Date(circleRead).toISOString()}` : ""}. Listed by Circle; we do not call them to check. They are paid through Circle Gateway, so the buyer needs a Gateway deposit on Arc.`,
      },
      results,

...
async function main() {
  ...
  console.log(JSON.stringify({ network: cfg.network, chainId: cfg.chainId, blocks: range, gaps, deploys, reverts, state, lastHourFees: fees[0], rpcLastHour: heads, liveLatencySecondsLastHour: latency[0] }, null, 2));
  await db.end();
}
main().catch((err) => {
  console.error(err);
